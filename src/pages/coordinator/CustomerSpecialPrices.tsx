...
import { coordinatorGetSpecialPrices, coordinatorSaveSpecialPrices } from '../../services/api/coordinatorApi';
...
export default function CoordinatorCustomerSpecialPrices() {
  ...
  const { data: productsData, isLoading: productsLoading, error: productsError } = useQuery({
    queryKey: ['coordinator-products-special-prices'],
  ...
  const { data: priceDetails, isLoading: priceDetailsLoading, error: priceDetailsError } = useQuery<PriceDetail[]>({
    queryKey: ['coordinator-customer-special-prices', id],
    queryFn: () => coordinatorGetSpecialPrices(id!),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    ...
    const productMap = new Map(productsData.items.map((p: Product) => [p.id, p]));
  ...
  const availableProducts = useMemo(() => {
    ...
    return all.filter((p: Product) => !selectedIds.has(p.id));
  ...
  const handleSelectProduct = (rowId: string, productId: string) => {
    const product = productsData?.items.find((p: Product) => p.id === productId);
    if (!product) return;
  ...
  const buildPayload = () => {
    return Object.entries(overrides).map(([productId, override]) => ({
      productId,
  ...
  const saveMut = useMutation({
    mutationFn: (payload: Array<{ productId: string; specialPrice?: number; discountPercent?: number }>) =>
      coordinatorSaveSpecialPrices(id!, payload),
    onSuccess: (_data, variables) => {
      ...
      if (id) queryClient.invalidateQueries({ queryKey: ['coordinator-customer-special-prices', id] });
    },
    onError: (e: any) => {
    ...
    <div className="animate-fade-in space-y-5 pb-20">
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="ml-auto inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 disabled:opacity-50"
        >
          ...
          <table className="w-full table-fixed text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={11} className="p-6 text-center text-red-500">Failed to load. Please refresh.</td>
                </tr>
              ) : (
                rows.map(row => {
                  ...
                  return (
                    <tr key={row.id} className="border-t border-slate-100">
                      ...
                      <td className="px-5 py-4 text-slate-600 text-center">{hasProduct ? quantity : '—'}</td>
                      <td className="px-5 py-4 text-slate-600 text-right">{hasProduct ? formatCurrency(base) : '—'}</td>
                      ...
                      <td className="px-5 py-4 text-slate-600 text-right">{hasProduct ? formatCurrency(discountAmount) : '—'}</td>
                      <td className="px-5 py-4 text-slate-600 text-right">{hasProduct ? (taxCode ?? '—') : '—'}</td>
                      <td className="px-5 py-4 text-slate-600 text-right">{hasProduct ? (taxAmountFromProduct != null ? formatCurrency(taxAmountFromProduct) : '—') : '—'}</td>
                      <td className={`px-5 py-4 text-sm font-semibold text-right ${hasOverride ? 'text-emerald-700' : 'text-slate-600'}`}>
      ...
      {activeDropdown && createPortal(
        <div
          ref={portalRef}
          style={dropdownStyle}
          className="z-[9999] fixed bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden"
        >
          {filteredProducts.length === 0 ? (
            ...
            <ul className="max-h-52 overflow-y-auto">
              {filteredProducts.map((p: Product) => (
                <li
                  key={p.id}
                  onMouseDown={() => handleSelectProduct(activeDropdown.rowId, p.id)}
                  className="px-4 py-2.5 cursor-pointer hover:bg-indigo-50 text-sm text-slate-800 border-b border-slate-50 last:border-0"
                >
